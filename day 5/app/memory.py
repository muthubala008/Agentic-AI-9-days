...
class ConversationStore:
    ...
    def load_history(self, thread_id: str) -> list[dict]:
        ...
        rows = self.conn.execute(
            "SELECT seq, role, text FROM message WHERE thread_id = ? ORDER BY seq ASC",
            (thread_id,),
        ).fetchall()
    ...
    def start_run(self, thread_id: str, model: str) -> str:
        ...
        self.conn.execute(
            "INSERT INTO run (id, thread_id, model, status) VALUES (?, ?, ?, 'running')",
    ...
    def record_tool_call(
        ...
        with self.transaction() as conn:
            cursor = conn.execute(
                "INSERT INTO run_step (run_id, seq, kind) VALUES (?, ?, 'tool') RETURNING id",
                (run_id, seq),
            )
            step_id = cursor.fetchone()["id"]
            conn.execute(
